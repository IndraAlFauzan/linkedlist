...
void List::addNode()/*Menambah sebuah Node kedalam list*/
{
	...
	/*Jika loop diatas dieksekusi, previous dan current akan menempati posisi dimana node baru akan ditambahkan diantara mereka .*/
...
bool List::Search(int nim, Node** previous, Node** current)/*Check apakah node yang dimaksud ada didalam list atau tidak*/
{
	*previous = START;
	*current = START;
	while ((*current != NULL) && (nim != (*current)->noMhs))
